#!/usr/bin/env python3
"""
Batch Experiment Runner for QUIC Dumbbell Topology
...
def run_experiment(bw, delay, loss, duration, output_dir):
    ...
    # print(f"\n{'='*70}")
    print(f"Running experiment: bw={bw}Mbps, delay={delay}, loss={loss}%")
...
def extract_results(output_dir):
    ...
    try:
        ...
        # Extract throughput from switches file
        throughput = 0
        with open(switches_file, 'r') as f:
            reader = csv.DictReader(f)
            for row in reader:
                # For dumbbell, we only have one interface (s1-eth21)
    ...
    for name, summary_file in summaries.items():
        if os.path.exists(summary_file):
            ...
            with open(summary_file, 'r') as f:
                reader = csv.DictReader(f)
                print(f"{'BW (Mbps)':<12} {'Delay':<10} {'Loss (%)':<10} {'Throughput (Mbps)':<20} {'Avg RTT (ms)':<15}")
                print(f"-" * 70)
                for row in reader:
                    print(f"{row['bandwidth_mbps']:<12} {row['delay']:<10} {row['loss_pct']:<10} "
                          f"{row['throughput_mbps']:<20} {row['avg_rtt_ms']:<15}")
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"QUIC DUMBBELL TOPOLOGY - BATCH EXPERIMENTS")
    print(f"{'='*70}")
